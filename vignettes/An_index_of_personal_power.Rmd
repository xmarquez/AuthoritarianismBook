...
And Polity IV has a measure of constraints on the executive, the `xconst` variable (excluding the transition/interruption/interregunum codes): 
...
The dataset of regime types by Magaloni, Chu, and Min [@MagaloniChuMin2013] contains two measures of personalism: one derived from the `xconst` variable of the Polity IV dataset, and another one (`lindex`) which they describe as follows:
...
We use this `lindex` variable only (since we directly include the `xconst` variable, noted above):
...
It is worh noting that because `persagg1ny` is invariant over the life of a regime, it is insensitive to fluctuations in personal power over time in it; by this measure, Lenin, Stalin, and Brezhnev all have the same amount of personal power over the life of the regime, since the "personalism" of the regime is averaged throughout its entire duration. Similarly, because `lindex` is always 1 at the beginning of a regime, the measure barely begins to capture Stalin's personal power as his tenure lengthened. Despite these problems, we nevertheless include both `lindex` and `persagg1ny` in the construction of the personal power index; in any case the model will discount them when they are highly uncorrelated with the rest of the other measures.
...
Before running the model, it is necessary to cut the continuous variables into a finite number of intervals. For entirely arbitrary reasons, I use 20 intervals. I also first transform the `lindex` and `persaggny` measures into percentile ranks. In the case of `lindex`, this smooths out some of the initial spikes of personal power.
