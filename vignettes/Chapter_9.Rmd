...
It also assumes you have the `dplyr`, `ggplot2`, `scales`, `forcats`, `ggalt`, `ggrepel`, `reshape2`, and `knitr` packages installed:
...
Repression and political regimes, 1949-2013. Shapes indicate the density of the distribution of regimes (darker means more concentration of regimes in this 2-dimensional space).The indicator of repression is from @Fariss2014; higher values indicate less repression. Regime data comes from @Kailitz2013.
...
War and democracy, 1816-2003. War data from Gledtisch's updated list of interstate wars [@Gleditsch2004]. A war in this dataset is an armed conflict with more than 1,000 deaths. Democracy data from @Pemstein2010, extended by the author [@Marquez2016].
...
data <- left_join(interstate_dyads_yearly, 
  ...
  mutate(index.x = pnorm(z1.x - 0.64), 
         ...
         ymax = pnorm(z1.y - 0.64 + 1.96*se.z1.y))
