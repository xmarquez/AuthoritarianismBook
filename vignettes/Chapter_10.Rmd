...
It also assumes you have the `dplyr`, `ggplot2`, `scales`, `forcats`, `broom`, `reshape2`, and `knitr` packages installed:
...
Here's an interactive version (requires the `highcharter` package):

```{r fig_10_6_interactive}
library(highcharter)

interactive_data <- data %>% 
  select(country_name, year, uds, mean_value, regime) %>%
  mutate(uds = round(uds,2), mean_value = round(mean_value,2),
         country_year = paste(country_name, year),
         regime = stringr::str_to_title(regime))

highchart() %>% 
  hc_add_series_df(interactive_data %>% 
                     filter(regime == "Communist"), # Comment out or change to see different regimes
                   type = "scatter", 
                   x = uds, 
                   y = mean_value, 
                   group = regime, 
                   name = country_year) %>% 
  hc_title(text = "Inequality and regime types") %>% 
  hc_tooltip(valueDecimals = 1,
             pointFormat = "{point.name} <br> UDS: {point.x} <br> Net Gini: {point.y}") %>% 
  hc_xAxis(title = list(text = "Unified Democracy Score"),
           plotLines = list(
             list(color = "#FF0000",
                  width = 2,
                  value = 0.5))) %>% 
  hc_yAxis(title = list(text = "Net Gini Index")) %>%
  hc_legend(title = list(text = "Regime Type"))
```
